...
public class RetryCapableBatchWriter implements BatchWriter{
    ...
    RetryCapableBatchWriter(final OpenGeminiClient openGeminiClient, final BiConsumer<Iterable<Point>, Throwable> exceptionHandler,
    ...
    private static final class WriteResult {
        ...
        private WriteResult(final OpenGeminiException e) {
            this.throwable = e;
            if (e.isRetryWorth()) {
    ...
    private WriteResult tryToWrite(final BatchPoints batchPoints) {
        try {
            openGeminiClient.write(batchPoints);
    ...
    private void evictTooOldFailedWrites() {
        while (usedRetryBufferCapacity > retryBufferCapacity && batchQueue.size() > 0) {
            ...
            exceptionHandler.accept(points,
                    new OpenGeminiException.RetryBufferOverrunException(
